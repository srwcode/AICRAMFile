...
const AssessmentEdit = () => {
  ...
  useEffect(() => {
    const fetchResultsAndAssessment = async () => {
      try {
        ...
        const resultsResponse = await fetch(`${config.API_URL}/results?assessment_id=${assessment_id}`, {
          headers: {
            'Content-Type': 'application/json',
            'token': token || ''
          }
        ...
        const resultItems = resultsData.result_items || [];
        const completedResults = resultItems.filter((result: Result) => result.status === 1);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dataToSubmit = hasCompletedResults 
        ? { name: formData.name } 
        : { ...formData, status: 1 };
      ...
      if (!response.ok) {
        if (responseData.error === 'matrix_error' ||
          ...
          throw new Error(responseData.error || 'Failed to update assessment');
        }
        return;
      }
  ...
  const disabledClass = "w-full text-gray-500 bg-gray-800 border border-gray-700 px-4 py-2.5 rounded-md cursor-not-allowed";
  ...
  return (
    <div className="rounded-lg border border-gray-700 bg-gray-800 shadow-lg">
      ...
      <div className="p-6 lg:p-8">
        {hasCompletedResults && (
          <div className="mb-8 bg-amber-900/20 border border-amber-500 text-amber-400 px-5 py-4 rounded-lg flex items-start">
            <AlertCircle className="w-auto mr-3 mt-1" size={18} />
            <div>
              <p className="font-medium mb-1">Limited editing mode</p>
              <p className="text-sm">This assessment has completed results. You can only modify the name.</p>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
                  ...
                  {organizations.length === 0 ? (
                    ...
                    organizations.map((organization) => (
                      <div 
                        ...
                        className={`p-4 rounded-lg border-2 ${hasCompletedResults ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'} transition duration-150 ${
            ...
            <div className="pb-10 border-b border-gray-700 space-y-5">
              <div>
                ...
                <textarea
                  ...
                  onChange={e => hasCompletedResults ? null : setFormData({ ...formData, situation: e.target.value })}
                  className={hasCompletedResults ? disabledClass + " h-32 resize-none" : inputClass + " h-32 resize-none"}
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={e => hasCompletedResults ? null : setFormData({ ...formData, constraint: e.target.value })}
                  className={hasCompletedResults ? disabledClass + " h-32 resize-none" : inputClass + " h-32 resize-none"}
            ...
            <div className="pb-10 border-b border-gray-700">
              <div className="flex gap-4 flex-wrap md:flex-nowrap">
                <div className="relative flex-grow">
                  <select
                    value={newAsset}
                    onChange={e => hasCompletedResults ? null : setNewAsset(e.target.value)}
                    className={hasCompletedResults ? disabledClass + " appearance-none pr-10" : inputClass + " appearance-none pr-10"}
                    disabled={hasCompletedResults}
                  >
                ...
                <button
                  ...
                  className={`bg-gray-800 hover:bg-gray-700 text-white px-6 py-2 rounded-md border border-gray-600 transition-colors ${hasCompletedResults ? 'opacity-50 cursor-not-allowed' : ''}`}
                  disabled={hasCompletedResults}
                >
                  Add
                </button>
              ...
              {newAsset === 'other' && !hasCompletedResults && (
                <input
                  type="text"
                  value={otherAsset}
            ...
            <div className="pb-10 border-b border-gray-700">
              <div className="flex gap-4 flex-wrap md:flex-nowrap">
                <div className="relative flex-grow">
                  <select
                    value={newThreat}
                    onChange={e => hasCompletedResults ? null : setNewThreat(e.target.value)}
                    className={hasCompletedResults ? disabledClass + " appearance-none pr-10" : inputClass + " appearance-none pr-10"}
                    disabled={hasCompletedResults}
                  >
              ...
              {newThreat === 'other' && !hasCompletedResults && (
                <input
                  type="text"
                  value={otherThreat}
          ...
          <div className="flex gap-4">
            ...
            >
              {hasCompletedResults ? "Update" : "Update and Analyze"}
            </button>
            <button
              type="button"
